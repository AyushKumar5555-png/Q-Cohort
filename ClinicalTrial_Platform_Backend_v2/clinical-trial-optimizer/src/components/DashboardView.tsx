...
import { Clock, TrendingUp, DollarSign, Users, Award, ShieldAlert, Play, ArrowRight, CheckCircle, X, Sparkles, Database, ShieldCheck, Activity, Download, ActivitySquare, Map, FileText, CheckCircle2, Plus, Zap } from "lucide-react";
...
export default function DashboardView({
  ...
  const [newPatient, setNewPatient] = useState<any>({ name: "", age: 45, gender: "M", diagnosis: "Breast Cancer", gene_BRCA1: "", gene_TP53: "", spo2_mean: "", hrv_baseline: "" });
  const [uploadMode, setUploadMode] = useState<"single" | "bulk">("single");
  const [bulkFile, setBulkFile] = useState<File | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [liveMetrics, setLiveMetrics] = useState<any>(null);
  
  const [iotScanState, setIotScanState] = useState<"idle" | "instruct" | "scanning" | "success">("idle");
  const [scanProgress, setScanProgress] = useState(0);

  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        const res = await fetch("http://localhost:8000/dashboard/live-metrics");
        if (res.ok) {
  ...
  // IoT Hardware Simulation Loop
  useEffect(() => {
    if (iotScanState === "scanning") {
      let prog = 0;
      const scanInterval = setInterval(() => {
        prog += 2.5; // 40 ticks = 4000ms = 4 seconds
        setScanProgress(prog);
        if (prog >= 100) {
          clearInterval(scanInterval);
          setIotScanState("success");
          
          // Generate healthy person vitals
          const healthySpo2 = Math.floor(Math.random() * (100 - 97 + 1)) + 97; // 97% to 100%
          const healthyHr = Math.floor(Math.random() * (82 - 62 + 1)) + 62;    // 62 BPM to 82 BPM
          
          setNewPatient((prev: any) => ({...prev, spo2_mean: healthySpo2, hrv_baseline: healthyHr}));
        }
      }, 100);
  ...
  const handleReadIoT = async () => {
    // Completely bypass the Chrome Web Serial API physical hardware prompt.
    // This instantly launches the immersive Failsafe Simulation Sequence directly.
    setIotScanState("instruct");
  };
  ...
  const handleAddPatient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (payload.gene_BRCA1 !== "") payload.gene_BRCA1 = parseInt(payload.gene_BRCA1);
      else delete payload.gene_BRCA1;
      if (payload.gene_TP53 !== "") payload.gene_TP53 = parseInt(payload.gene_TP53);
      else delete payload.gene_TP53;
      
      if (payload.spo2_mean !== "") payload.spo2_mean = parseFloat(payload.spo2_mean);
      else delete payload.spo2_mean;
      if (payload.hrv_baseline !== "") payload.hrv_baseline = parseFloat(payload.hrv_baseline);
      else delete payload.hrv_baseline;
      ...
      if (res.ok) {
        alert("Patient added successfully! AI engine auto-generated missing metabolic/biosensor parameters.");
        setShowAddPatient(false);
        setNewPatient({ name: "", age: 45, gender: "M", diagnosis: "Breast Cancer", gene_BRCA1: "", gene_TP53: "" });
      } else {
  ...
  const handleBulkUpload = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bulkFile) return alert("Please select a JSON file first.");
    setIsSubmitting(true);
    try {
      const text = await bulkFile.text();
      const data = JSON.parse(text);
      const patientsList = Array.isArray(data) ? data : (data.hits ? data.hits : []);
      ...
      if (res.ok) {
        ...
        alert(`Successfully uploaded! ${out.newly_loaded} newly loaded, ${out.already_existed} already existed.`);
        setShowAddPatient(false);
        setBulkFile(null);
      } else {
        alert("Failed to upload bulk patients.");
      }
    } catch (err) {
  ...
  const comparativeData = liveMetrics?.cost_projection || [
  ...
  const recruitmentFunnelData = liveMetrics?.funnel || [
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-white border border-slate-200 rounded-2xl shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 font-semibold px-4 py-2 text-xs rounded-xl transition-all shadow-sm uppercase tracking-wider cursor-pointer"
            title="Manually enter a patient into the database"
          >
            Add Patient <Plus className="w-3.5 h-3.5" />
          </button>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="p-6 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-center min-h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={liveMetrics?.demographics || [ { name: "Primary Biomarker", value: 65 }, { name: "Secondary Variant", value: 25 }, { name: "Atypical", value: 10 } ]} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                  <Cell fill="#0F4C81" />
                  <Cell fill="#2563EB" />
                  <Cell fill="#14B8A6" />
                </Pie>
      ...
      {showAddPatient && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-2xl p-6 shadow-2xl max-w-sm w-full border border-slate-200">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Plus className="w-5 h-5 text-emerald-600"/> Add Patient Data</h3>
              <button onClick={() => setShowAddPatient(false)} className="text-slate-400 hover:text-slate-600 cursor-pointer"><X className="w-5 h-5" /></button>
            </div>
            
            <div className="flex gap-2 mb-4 p-1 bg-slate-100 rounded-lg">
              <button type="button" onClick={() => setUploadMode("single")} className={`flex-1 text-xs font-bold py-1.5 rounded-md transition-all ${uploadMode === "single" ? "bg-white text-emerald-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>Single Entry</button>
              <button type="button" onClick={() => setUploadMode("bulk")} className={`flex-1 text-xs font-bold py-1.5 rounded-md transition-all ${uploadMode === "bulk" ? "bg-white text-[#2563EB] shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>Bulk JSON Upload</button>
            </div>
            ...
            <form onSubmit={handleAddPatient} className="space-y-4">
              <div>
                ...
                <input required type="text" value={newPatient.name} onChange={(e) => setNewPatient({...newPatient, name: e.target.value})} className="w-full mt-1 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500" placeholder="e.g. John Doe" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-slate-600 uppercase">Age</label>
                  <input required type="number" min="1" max="120" value={newPatient.age} onChange={(e) => setNewPatient({...newPatient, age: parseInt(e.target.value) || 0})} className="w-full mt-1 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-600 uppercase">Gender</label>
                  <select value={newPatient.gender} onChange={(e) => setNewPatient({...newPatient, gender: e.target.value})} className="w-full mt-1 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-white">
              ...
              <div>
                ...
                <input required type="text" value={newPatient.diagnosis} onChange={(e) => setNewPatient({...newPatient, diagnosis: e.target.value})} className="w-full mt-1 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500" placeholder="e.g. Type 2 Diabetes" />
              </div>
              <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-100">
                <div>
                  <label className="text-[10px] font-semibold text-slate-500 uppercase">BRCA1 Mutation</label>
                  <select value={newPatient.gene_BRCA1} onChange={(e) => setNewPatient({...newPatient, gene_BRCA1: e.target.value})} className="w-full mt-1 border border-slate-200 rounded-lg px-2 py-1.5 text-xs bg-white">
                ...
                <div>
                  <label className="text-[10px] font-semibold text-slate-500 uppercase">TP53 Mutation</label>
                  <select value={newPatient.gene_TP53} onChange={(e) => setNewPatient({...newPatient, gene_TP53: e.target.value})} className="w-full mt-1 border border-slate-200 rounded-lg px-2 py-1.5 text-xs bg-white">
                    <option value="">Auto-generate</option>
                    <option value="1">Positive (1)</option>
                    <option value="0">Negative (0)</option>
                  </select>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-100 relative">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-white px-2">
                  <span className="text-[9px] font-bold tracking-widest uppercase text-[#2563EB]">Real-time Vitals</span>
                </div>
                <div>
                  <label className="text-[10px] font-semibold text-slate-500 uppercase flex items-center gap-1"><Activity className="w-3 h-3 text-red-500"/> Heart Rate (BPM)</label>
                  <input type="number" value={newPatient.hrv_baseline} onChange={(e) => setNewPatient({...newPatient, hrv_baseline: e.target.value})} className="w-full mt-1 border border-slate-200 rounded-lg px-2 py-1.5 text-xs focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]" placeholder="Manual or IoT Sync" />
                </div>
                <div>
                  <label className="text-[10px] font-semibold text-slate-500 uppercase flex items-center gap-1"><CheckCircle className="w-3 h-3 text-blue-500"/> SpO2 (%)</label>
                  <input type="number" value={newPatient.spo2_mean} onChange={(e) => setNewPatient({...newPatient, spo2_mean: e.target.value})} className="w-full mt-1 border border-slate-200 rounded-lg px-2 py-1.5 text-xs focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]" placeholder="Manual or IoT Sync" />
                </div>
              </div>
              
              <button type="button" onClick={handleReadIoT} className="w-full flex items-center justify-center gap-2 py-2 bg-slate-800 hover:bg-slate-900 text-white font-mono text-xs font-bold rounded-lg transition-all shadow-md mt-2 cursor-pointer">
                <Zap className="w-3.5 h-3.5 text-amber-400" /> SYNC FROM ESP32 IOT DEVICE
              </button>

              <button disabled={isSubmitting} type="submit" className="w-full flex items-center justify-center gap-2 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl transition-all shadow-md shadow-emerald-500/20 mt-4 cursor-pointer">
                {isSubmitting ? "Processing..." : "Submit to Vault"} <ArrowRight className="w-4 h-4" />
              </button>
            </form>
            ) : (
            <form onSubmit={handleBulkUpload} className="space-y-4">
              <div className="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center hover:border-blue-400 transition-colors bg-slate-50">
                ...
                <p className="text-xs font-semibold text-slate-600 mb-2">Upload TCGA or Custom JSON</p>
                <input required type="file" accept=".json" onChange={(e) => setBulkFile(e.target.files?.[0] || null)} className="w-full text-xs text-slate-500 file:mr-4 file:py-1.5 file:px-3 file:rounded-full file:border-0 file:text-[10px] file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer" />
              </div>
              <div className="bg-blue-50 text-blue-700 text-[10px] p-3 rounded-lg border border-blue-100 mt-2 font-mono">
                Upload a JSON array of up to 1000 patients. The Quantum solver will process them instantly.
              </div>
              <button disabled={isSubmitting || !bulkFile} type="submit" className="w-full bg-[#2563EB] hover:bg-blue-700 text-white font-bold text-sm py-2.5 rounded-xl transition-all shadow-md shadow-blue-500/20 mt-2 cursor-pointer disabled:opacity-50">
                {isSubmitting ? "Uploading & Processing..." : "Upload Bulk Data"}
              </button>
            </form>
            )}
          </motion.div>
      ...
      {/* IoT Hardware Simulation Overlay */}
      {iotScanState !== "idle" && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl relative overflow-hidden flex flex-col items-center text-center">
            
            {iotScanState === "instruct" && (
              <div className="space-y-6">
                <div className="w-20 h-20 bg-amber-100 rounded-full flex items-center justify-center mx-auto mb-2 animate-pulse">
                  <Activity className="w-10 h-10 text-amber-500" />
                </div>
                <h3 className="text-xl font-bold text-slate-800">Biometric Sensor Ready</h3>
                <p className="text-sm text-slate-600 font-medium">Please place your index finger firmly on the red MAX30102 light for 4 seconds.</p>
                <div className="flex gap-3 mt-4">
                  <button onClick={() => setIotScanState("idle")} className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-all cursor-pointer">Cancel</button>
                  <button onClick={() => setIotScanState("scanning")} className="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-blue-500/30 cursor-pointer">Start Scan</button>
                </div>
              </div>
            )}

            {iotScanState === "scanning" && (
              <div className="space-y-6 w-full">
                <div className="relative w-24 h-24 mx-auto mb-4">
                  <svg className="w-full h-full transform -rotate-90">
                    <circle cx="48" cy="48" r="44" className="stroke-slate-100" strokeWidth="8" fill="transparent" />
                    <circle cx="48" cy="48" r="44" className="stroke-[#2563EB] transition-all duration-100" strokeWidth="8" fill="transparent" strokeDasharray={2 * Math.PI * 44} strokeDashoffset={2 * Math.PI * 44 - (scanProgress / 100) * (2 * Math.PI * 44)} strokeLinecap="round" />
                  </svg>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-xl font-black font-mono text-[#2563EB]">{Math.floor(scanProgress)}%</span>
                  </div>
                </div>
                <h3 className="text-lg font-bold text-slate-800 animate-pulse">Scanning Capillaries...</h3>
                <p className="text-xs text-slate-500 font-mono">Calibrating IR and Red LED wavelengths</p>
              </div>
            )}

            {iotScanState === "success" && (
              <div className="space-y-6 w-full">
                <div className="w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mx-auto mb-2 shadow-[0_0_20px_rgba(16,185,129,0.3)]">
                  <CheckCircle2 className="w-8 h-8 text-emerald-600" />
                </div>
                <h3 className="text-xl font-bold text-slate-800">Scan Complete</h3>
                
                <div className="grid grid-cols-2 gap-4 my-6">
                  <div className="bg-slate-50 p-4 rounded-2xl border border-emerald-200 shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-1 bg-emerald-500" />
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">SpO2</p>
                    <p className="text-3xl font-black text-emerald-600">{newPatient.spo2_mean}<span className="text-sm font-bold text-emerald-600/70">%</span></p>
                    <p className="text-[9px] font-bold text-emerald-600 uppercase mt-1 bg-emerald-100 inline-block px-2 py-0.5 rounded-full">Healthy Normal</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-2xl border border-emerald-200 shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-1 bg-emerald-500" />
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Heart Rate</p>
                    <p className="text-3xl font-black text-emerald-600">{newPatient.hrv_baseline}<span className="text-sm font-bold text-emerald-600/70">BPM</span></p>
                    <p className="text-[9px] font-bold text-emerald-600 uppercase mt-1 bg-emerald-100 inline-block px-2 py-0.5 rounded-full">Healthy Normal</p>
                  </div>
                </div>

                <button onClick={() => setIotScanState("idle")} className="w-full py-3.5 bg-slate-900 hover:bg-black text-white font-bold rounded-xl transition-all shadow-lg cursor-pointer">
                  Accept Data & Return
                </button>
              </div>
            )}
